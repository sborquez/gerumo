...
def filter_dataset(dataset, telescopes=[], number_of_observations=[], domain={}):
    ...
    # filter domain
    selection = None
    for target, (min_val, max_val) in domain.items():
        if selection is None:
            selection = (min_val <= filtered_dataset[target]) & (filtered_dataset[target] <= max_val)
        else:
            selection |= (min_val <= filtered_dataset[target]) & (filtered_dataset[target] <= max_val)
    if selection is None:
...
def get_target_shape(target_names, target_mode, target_mode_config):
    pass
...
def probability_map(target_values, target_names, target_shape, target_range, target_resolutions, target_sigmas):
    ...
    size and the value that represent each cell is given by the target_ranges and
    target_resolutions.
    ...
    config = {
        ...
        target_shape = [a, b, c] //related with target_names
    }
    ...
    if not hasattr(probability_map, "normal_dist"):
        probability_map.normal_dist = get_normal_distribution(shape_array=target_shape_arr, sigmas=target_sigmas)

    normal_dist_centers = target_shape_arr
...
def one_cell(target_values, target_names, target_shape, target_range, target_resolutions):
    y = np.zeros((len(target_values), *target_shape), dtype=float)
    target_range_arr = np.array(target_range)
    target_resolutions_arr = np.array(target_resolutions)
    indices = ((target_values - target_range_arr[:,0])/target_resolutions_arr).astype(int)
...
def two_outputs_one_cell(target_values, target_names, target_to_output, target_shape, target_range, target_resolutions):
    raise NotImplementedError

def targets_to_matrix(targets_values, 
                      target_names=["alt", "az", "log10_mc_energy"], 
                      target_mode="probability_map",
                      target_mode_config={"target_shape": (10, 10, 10),
                                          "target_range": [(1.04, 1.39), (-0.52, 0.52), (-1.74, 2.44)],
